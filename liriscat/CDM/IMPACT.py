...
class IMPACT(AbstractModel):
    ...
    def init_model(self, train_data: Dataset, valid_data: Dataset):
        ...
        if self.config['low_mem'] == True:
            self.model = IMPACTModel_low_mem(train_data.n_users, train_data.n_questions, train_data.n_categories,
                                             self.concept_map,
                                             train_data, valid_data, self.config['d_in'], self.config['num_responses'])
            self.loss = custom_loss_low_mem
        else:
            self.model = IMPACTModel(train_data.n_users, train_data.n_questions, train_data.n_categories, self.concept_map,
