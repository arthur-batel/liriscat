...
def objective_hps(trial, config, train_data, valid_data):
    meta_lr = trial.suggest_float('meta_lr', 0.001, 0.07, log=True)
    kl_weight = trial.suggest_float('kl_weight', 1e-4, 1e-3, log=True)
...
def main(dataset_name, nb_trials):
    ...
    config = micat.utils.generate_eval_config(dataset_name=dataset_name,load_params=True, CDM='ncdm',esc = 'error', valid_metric= 'rmse', pred_metrics = ["mi_acc"], profile_metrics = ['meta_doa'], save_params=False, n_query=6, num_epochs=100, patience = 20, num_inner_users_epochs=3, meta_trainer='BETA-CD', batch_size=512, lambda_=9.972254466547545e-06, inner_user_lr=2.10315236650495e-05, num_responses=12)
