...
def launch_test(trial, train_data, valid_data, config):
    ...
    logging.info(f"Trial {trial.number}; Best pareto_index: {pi}; Results: {d}")
...
def objective_hps(trial, config, train_data, valid_data):
    ...
    config['learning_users_emb_lr'] = learning_users_emb_lr
...
def main(dataset_name, nb_trials):
    ...
    config = micat.utils.generate_eval_config(dataset_name=dataset_name,load_params=True, esc = 'error', valid_metric= 'mi_acc', pred_metrics = ["mi_acc"], profile_metrics = ['meta_doa'], save_params=False, n_query=6, num_epochs=100, patience = 20, num_inner_users_epochs=3, meta_trainer='GAP', batch_size=512, lambda_=2.2656270501845414e-06, inner_user_lr=0.0007838402204978467, num_responses=12)
    ...
    storage_name = f"sqlite:///{dataset_name}_IMPACT_GAP.db"
    study_name = f"hps_parallel_{dataset_name}_IMPACT_GAP"
