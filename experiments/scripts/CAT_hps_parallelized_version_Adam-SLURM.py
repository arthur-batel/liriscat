...
def objective_hps(trial, config, train_data, valid_data):
    lr = trial.suggest_float('inner_user_lr', 1e-5, 5e-2, log=True)
    iue = trial.suggest_int('num_inner_users_epochs', 3, 10)
    
    config['inner_user_lr'] = lr
    config['num_inner_users_epochs'] = iue
